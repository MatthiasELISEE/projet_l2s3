...
public class Joueur {
	...
	boolean assecher(int i, int j) {
		if (valide(i, j)) {
			if (this.modele.zones[i][j].etat == 1) {
			this.modele.zones[i][j].assecher();
			this.modele.zones[i][j].evolue();
			return true;
			} else {
	...
	boolean deplacer(int i, int j) {
		if (valide(i, j)) {
			if (this.modele.zones[i][j].etat > 0) {
				this.modele.zones[this.x][this.y].retirerJoueur(this);
				this.modele.zones[i][j].ajouterJoueur(this);
				this.x = i;
				this.y = j;
				return true;
			} else {
